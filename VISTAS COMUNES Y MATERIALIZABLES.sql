...
-- El resultado de las vistas materializables se actualizanperiódicamente
-- y puede ser ante las siguientes situaciones:
--     ? Automáticamente cuando se le hace commit a alguna de las
--       tablas bases.
--     ? Automáticamente en un horario predefinido.
--     ? En forma manual.
...
--    ? Completa: Implica, por cada actualización, borrar todos los datos
--       de la vista y volverla a cargar de datos.
--    ? Fast: A través de un log de movimientos de la vista, la
--       actualización de la misma va a ser incremental, es decir, va a
--       actualizar los últimos cambios hechos sobre ella.
--    ? Force: El sistema define si lo hace Fast o Completa.
